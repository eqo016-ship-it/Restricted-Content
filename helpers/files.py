...
async def fileSizeLimit(file_size, message, action_type="unduh", is_premium=False):
    MAX_FILE_SIZE = 2 * 2097152000 if is_premium else 2097152000
    if file_size > MAX_FILE_SIZE:
        await message.reply(
            f"Ukuran file melebihi batas {get_readable_file_size(MAX_FILE_SIZE)} dan tidak bisa di{action_type}."
        )
        return False
    return True